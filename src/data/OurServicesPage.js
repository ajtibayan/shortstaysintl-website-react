export const serviceItems = [
  {
    title: "Consulting",
    divClass: "consulting",
    image: "/images/ourservices/service-consulting.jpg",
    description: [
      {
        subHeading: "",
        bullets: [
          "You’re an investor looking to find out your rental options before purchasing the property",
          "Curious to see how much a property can generate doing Short Term or Mid Term Rental?",
          "Unsure of the by-laws and permit requirements for a certain property?",
          "Unsure of the type of property and location that can get the highest return for your investment",
          "Unsure of the occupancy rate and what’s the ROI on your property?",
          "Contact us for a free GUIDE on how to get started and a free consultation.",
          "A 5-10 page document providing a thorough market & rental analysis is available for $500. Includes full analysis of your local area, by-laws & zoning, major competitions, suggested pricing strategy, average occupancy and best rental options available. This fee will be given ack to you if you decide to hire us on as your property Short-Term or Mid-Term Co-Hosts/Management Team.",
        ],
      },
    ],
  },
  {
    title:
      "Account & Listing Set-up: $300 for Airbnb ($100 on additional platforms)",
    divClass: "listing",
    image: "/images/ourservices/service-listing.jpg",
    description: [
      {
        subHeading: "",
        bullets: [
          "Help you create an Admin Hosting account + set up payout",
          "End-to-end listing creation",
          "Listing optimization",
          "Search engine optimization",
          "Multiple platforms",
        ],
      },
    ],
  },
  {
    title:
      "Co-hosting & Guests Management: 15-20% of revenue after cleaning fees",
    divClass: "management",
    image: "/images/ourservices/service-management.jpg",
    description: [
      {
        subHeading: "Online Management",
        bullets: [
          "Reservation management across platforms",
          "Guest screening & communication",
          "Income maximizing pricing",
          "24/7 Concierge service",
        ],
      },
      {
        subHeading: "Check-in",
        bullets: [
          "Guest welcoming",
          "Key delivery",
          "Supplies restocking",
          "Automated scheduling",
        ],
      },
      {
        subHeading: "Check-out",
        bullets: [
          "Cleaning",
          "Laundry",
          "Property inspection",
          "Automated scheduling",
        ],
      },
      {
        subHeading: "Property Management",
        bullets: [
          "Coordinate with local specialized professionals (pool,hot tub,sauna, lawn care, snow removal)",
          "Laundry",
          "Coordinate with local handyman to do minor jobs",
          "Available for small and emergency jobs relating to the upkeep of the property",
        ],
      },
      {
        subHeading: "Accounting",
        bullets: ["Tracking of reservations & payout", "Tracking of expenses"],
      },
    ],
  },
  {
    title: "Travel Coordination",
    divClass: "travel",
    image: "/images/ourservices/service-travel.jpg",
    description: [
      {
        subHeading: "travel",
        bullets: [
          'Mai Nguyen & Jonathan Lim are also founders of Travel With the Lims, a TICO certified travel company associated with The FrontLiners Agency that specializes in group trips, ski & snowboarding trips. Interested in their travel concierge services? Email them at <a href="mailto:travelwiththelims@gmail.com">travelwiththelims@gmail.com</a> and a Travel Pro on their team can help you.',
        ],
      },
    ],
  },
  {
    title: "Financing",
    divClass: "financing",
    image: "/images/ourservices/service-financing.jpg",
    description: [
      {
        subHeading: "financing",
        bullets: [
          'Jonathan Lim is a certified Mortgage Specialist with Centum. He and  his team of  Mortgage professionals can also help you find various solutions to finance your property. For more details, contact Jonathan Lim at <a href="mailto:info@limmanagement.com">info@limmanagement.com</a>.',
        ],
      },
    ],
  },
];
